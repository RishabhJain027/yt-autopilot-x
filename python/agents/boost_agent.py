...
CTR thumbnail concepts, and YouTube SEO ranking factors.
"""
...
class BoostAgent:
    def evaluate_hook_strength(self, hook_text: str) -> Dict[str, Any]:
        ...
        # Power words that drive Shorts retention
        power_words = ["new", "secret", "stop", "never", "everyone", "shocking", "breakthrough", "free", "faster", "insane", "hugging face", "open source", "ai", "leak"]
        matches = [w for w in words if w.lower().strip(".,!?:") in power_words]
        
        # Scoring logic
        length_score = 100 if 5 <= word_count <= 14 else (70 if word_count < 5 else 60)
        power_score = min(100, len(matches) * 35)
        urgency_score = 90 if any(char in hook_text for char in ["!", "?"]) else 75
